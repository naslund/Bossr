...
namespace Bossr.Api.Repositories
{
    ...
    public class RaidRepository : IRaidRepository
    {
        ...
        public async Task CreateAsync(IRaid raid)
        {
            ...
            {
                raid.Id = await conn.QuerySingleAsync<int>("INSERT INTO Raids (FrequencyHoursMin, FrequencyHoursMax) VALUES (@FrequencyHoursMin, @FrequencyHoursMax) SELECT CAST(SCOPE_IDENTITY() as int)", raid);
            }
        }
        ...
        public async Task UpdateAsync(IRaid raid)
        {
            using (var conn = dbConnectionFactory.CreateConnection())
            {
                await conn.ExecuteAsync("UPDATE Raids SET FrequencyHoursMin = @FrequencyHoursMin, FrequencyHoursMax = @FrequencyHoursMax WHERE Id = @Id", raid);
            }
        }
    }
}
